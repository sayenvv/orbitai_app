...
export type ResearchCompanionWorkspaceTab = {
  ...
  insightTypesParam: string | null;
  draftId: string;
  hasUnsavedChanges?: boolean;
};

export type ResearchCompanionMoreMenuItem = PhotoStudioMoreMenuItem;

type ResearchCompanionWorkspaceChromeProps = {
  tabs: ResearchCompanionWorkspaceTab[];
  activeTabId: string;
  ...
  onHomeClick?: () => void;
  moreMenuItems?: ResearchCompanionMoreMenuItem[];
  onOpenHelp?: () => void;
};

export function ResearchCompanionWorkspaceChrome({
  tabs,
  activeTabId,
  onSelectTab,
  onCloseTab,
  ...
  moreMenuItems = [],
  onOpenHelp,
}: ResearchCompanionWorkspaceChromeProps) {
  return (
    <header className="relative z-[110] shrink-0 border-b border-border/40 bg-card/80 backdrop-blur-md">
      <div className="flex h-11 items-stretch gap-0 px-3 sm:px-4">
        <nav className="flex shrink-0 items-center" aria-label="Clovai Insights navigation">
