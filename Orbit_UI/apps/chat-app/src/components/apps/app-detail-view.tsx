...
export function AppDetailView({ app }: { app: CatalogApp }) {
  ...
  const related = useMemo(
    () =>
      sortAppsByAvailability(
        visibleAppsCatalog.filter((candidate) => candidate.id !== app.id),
        isAuthenticated,
      ).slice(0, 3),
            ...
            <div className="relative grid gap-6 lg:grid-cols-[1.08fr_0.92fr] lg:items-center">
              <div className="space-y-4 text-white">
                ...
                {blockReason ? (
                  ...
                  <Link href={launchHref} className={primaryActionClassName}>
                    <ArrowRight className="h-4 w-4" />
                    {primaryActionLabel}
                  </Link>
                ) : (
                  <button type="button" disabled className={primaryActionClassName}>
                    {primaryActionLabel}
                  </button>
                )}
              </div>
